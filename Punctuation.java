 ...
 * with punctuation in it. The punctuation will be included in the word.
 * The punctuation being checked will be ' " : ; , . ? and !
 * 
 * Working on: Using String Methods
 * 
 * Pseudocode:
 * 
 * class header
 ...
 * 	creates instance of Punctuation(), calls searchIt(text)
 * 
 * searchIt(text)
 * 	call getPunctuationWords(text), set it equal to punctuationWords
 * 	call printWords(punctuationWords)
 * 	
 * 
 * getPunctuationWords(text)
 * 	Trim the text(in case of spaces at front or end)
 * 	Seperate the entire text into words using substring and checking for spaces
 * 	call checkForPunctuation(word) for each word
 * 	if checkForPunctuation is true, add the word with punctuation to punctuationWords
 * 
 * checkForPunctuation(word)
 * 	if the word has one of these characters: '"?!.,:;    return true
 * 	otherwise return false
 * 	
 * 
 * printWords(punctuationWords)
 * 	print out punctuationWords
 * 
 * 
 * 
 * 
...
public class Punctuation
{
	...
	public static void main(String[] args)
	{
		...
		//Holds text value, creates instance of Punctuation, calls searchIt
	...
	{
		//Calls getPunctuationWords and printWords
	...
	{
		//Seperates the text into words, checks if the words have punctuation in it using checkForPunctuation()
		//If yes punctuation, adds the word to the string wordsWithPunctuation
		int firstCharPos = 0;
		int lastCharPos = 0;
		String word = "";
		String wordsWithPunctuation = "";
		boolean hasPunctuation = false;
		
		text.trim();
		text = text + " ";
		while(text.length() > firstCharPos)
		{
			lastCharPos = text.indexOf(' ', firstCharPos);
			word = text.substring(firstCharPos, lastCharPos);
			firstCharPos = lastCharPos + 1;
			hasPunctuation = checkForPunctuation(word);
			if(hasPunctuation)
				wordsWithPunctuation = wordsWithPunctuation + word + "\n";
		}
	...
	{
		//Checks if the word has punctuation and returns a boolean based on if it does or not.
		String punctuationChecker = "\"\'?!.,:;";
		for(int i = 0; i < word.length(); i++)
		{
			for(int j = 0; j < punctuationChecker.length(); j++)
			{
				if(word.charAt(i) == punctuationChecker.charAt(j))
					return true;
			}
		}
	...
	public void printWords(String punctuationText)
	{
		//Prints the words with punctuation
		System.out.print("Here are all of the words with punctuation: \n" + punctuationText);
	}
}
